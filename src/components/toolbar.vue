<template>
  <div class="toolbar">
    ...
    <el-dialog title="添加节点" width="50%" :visible.sync="addNodeDialogVisible">
      <!--hello-->

      <el-form title="节点内容" inline="true">
    ...
    <el-dialog title="删除节点" width="50%" :visible.sync="deleteNodeDialogVisible">
      hello
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  data: () => ({
    tools: [
      ...
      { icon: 'mdi-plus-thick', tip: '添加', event: 'addNodeButton' }, //添加增删节点功能
      // { icon: 'mdi-minus-thick', tip: '删除节点', event: 'deleteNodeButton' },
  ...
  methods: {
    ...
    submitFormData () {
      let _this = this
      // const newegde = JSON.parse(this.formData)
      // let new_edge = {
      //
      // }
      // this.initializeObj(defaultEdge, newegde)
      const model = {
        id: 'node',
        label: 'node',
        address: 'cq',
      ...
      const newedge = {
        ...
        label: _this.formData.label,

      }

      // _this.graph.addItem('node', model);
      _this.graph.addItem('edge', newedge);
      // _this.graph.addItem('edge', newegde)
      // _this.$store.commit('addEdge', newegde)
    },
    saveImage () {
